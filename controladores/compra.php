...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idcompra)){
			$rspta=$compra->insertar($idproveedor,$idpersonal,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$_POST["idproducto"],$_POST["cantidad"],$_POST["precio_compra"],$_POST["precio_venta"]);
	...
	case 'listarArticulos':
		...
		while ($reg=$rspta->fetch_object()) {
            ...
            "5"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idproducto.',\''.$reg->nombre.'\')"><span class="fa fa-plus"></span></button>'
